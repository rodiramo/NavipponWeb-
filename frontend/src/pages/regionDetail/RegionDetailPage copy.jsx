...
import HorizontalExperienceCard from "../experience/container/HorizontalExperienceCard";
...
// Enhanced region data with coordinates and weather info
const regionData = {
  Hokkaido: {
    name: "Hokkaido",
    image: HokkaidoImage,
...
const RegionDetail = () => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(
        (exp) =>
          ...
          (exp.caption &&
            exp.caption.toLowerCase().includes(searchTerm.toLowerCase()))
      );
    }
  ...
  const fetchRegionData = async () => {
    try {
      ...
      // Fetch experiences and stats for this region
  ...
  return (
    <MainLayout>
      ...
      <BgShape />
      <Container sx={{ mt: 3, mb: 4, position: "relative", zIndex: 1 }}>
        {/* Region Information & Weather */}
        <Grid container spacing={4} sx={{ mb: 6 }}>
          <Grid item xs={12} md={8}>
            <Card
              elevation={0}
              sx={{ p: 4, border: `1px solid ${theme.palette.primary.main}15` }}
            >
