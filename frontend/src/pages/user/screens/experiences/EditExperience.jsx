...
const EditExperience = () => {
  ...
  const { data, isLoading, isError } = useQuery({
    queryFn: () => getSingleUserExperience({ slug, token: jwt }),
    queryKey: ["userExperience", slug],
    onSuccess: (data) => {
  ...
  const {
    ...
    onSuccess: (data) => {
      queryClient.invalidateQueries(["userExperience", slug]);
      toast.success("Experiencia actualizada");
      navigate(`/user/experiences/manage/edit/${data.slug}`, { replace: true });
    },
    onError: (error) => {
  ...
  const handleUpdateExperience = async () => {
    ...
    updatedData.append(
      "document",
      JSON.stringify({ body, categories, title, tags, slug: experienceSlug, caption, region, prefecture, price })  
    );
